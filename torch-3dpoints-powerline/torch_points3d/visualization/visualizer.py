...
class Visualizer(object):
    ...
    def __init__(self, viz_conf, num_batches, batch_size, save_dir, tracker):
        ...
        if "predictions" in self._format:
            self._save_preds = True
            self._pred_dir = save_dir
        else:
    ...
    def save_visuals(self, visuals):
        """This function is responsible to save the data into .ply objects
            Parameters:
                visuals (Dict[Data(pos=torch.Tensor, ...)]) -- Contains a dictionary of tensors
            ...
            for idx in np.argwhere(self._seen_batch == batch_indices).flatten():
                ...
                for visual_name, item in visuals.items():
                    ...
                    if hasattr(self, "_viz_path"):
                        ...
                        filename = "{}_{}.txt".format(self._seen_batch, pos_idx)
                    ...
                    if hasattr(self, "_writer"):
                        ...
                        for label, k in self._tensorboard_mesh.items():
                            ...
                            if len(value.shape) == 2 and value.shape[1] == 3:
                                ...
                                value = np.tile((255 * colors(value.numpy() % 10))[:, 0:3].astype(np.uint8),
                                                (pos.shape[0], 1)).reshape((1, -1, 3))
                            elif len(value.shape) == 1 or value.shape[1] == 1:
                                value = (255 * colors(value.numpy() % 10))[:, 0:3].astype(np.uint8).reshape((1, -1, 3))
                            else:
                                continue

                            self._writer.add_mesh(self._stage + "/" + visual_name + "/" + label, pos, colors=value,
                                                  config_dict=config_dict, global_step=(self._current_epoch - 1) * (
                                        10 ** ceil(log10(stage_num_batches + 1))) + self._seen_batch)

            # if self._save_preds:
            #     ipdb.set_trace()
            #     for visual_name, item in visuals.items():
            #         np.set_printoptions(suppress=True)
            #         pred = np.around(item.pred.detach().cpu().numpy(),0)
            #         pos = item.pos.detach().cpu().numpy()
            #         # pos += np.median(pos, 0)
            #
            #         pred = pred[:,np.newaxis]
            #         store_data = np.round(np.concatenate((pos,pred),axis=1), 3)
            #         print(store_data)
            #         # with open(os.path.join(self._pred_dir, f"{self._stage}_{visual_name}_preds.csv"), 'a') as file:
            #         #     np.savetxt(file,store_data, delimiter=" ", fmt= '%.04f')
            #         with open(os.path.join(self._pred_dir, f"{self._stage}_{visual_name}_preds.txt"), 'a') as file:
            #             np.savetxt(file,store_data, delimiter=" ", fmt= '%.04f %.04f %.04f %i')
