...
from ..PointNetSharp import NeighborPointInteractX
...
class SimpleBlock(BaseModule):
    ...
    def __init__(
        ...
        self.sharp_conv = sharp_conv
        self.k = max_num_neighbors
        if sharp_conv:
            density_parameter = self.RIGID_DENSITY  # TODO not sure about this one
            self.point_conv = NeighborPointInteractX(
                in_dim=num_inputs, out_dim=num_outputs, agg=sharp_conv_agg,
                n_coord=3, neighbor_pos=sharp_neighbor_pos,
            )
        else:
            if deformable:
                density_parameter = self.DEFORMABLE_DENSITY
                self.point_conv = KPConvDeformableLayer(
                    num_inputs, num_outputs, point_influence=prev_grid_size * sigma, add_one=add_one, **kwargs
                )
            else:
                density_parameter = self.RIGID_DENSITY
                self.point_conv = KPConvLayer(
                    num_inputs, num_outputs, point_influence=prev_grid_size * sigma, add_one=add_one, **kwargs
                )
    ...
    def forward(self, data, precomputed=None, **kwargs):
        ...
        if precomputed:
            ...
            q_x = data.x[query_data.batch]
        else:
            q_pos, q_x, q_batch = query_data.pos, query_data.x, query_data.batch
        ...
        if self.sharp_conv:
            x = self.point_conv(q_pos, data.pos, idx_neighboors, q_x, data.x,)
        else:
            x = self.point_conv(q_pos, data.pos, idx_neighboors, data.x, )
        if self.bn:
            x = self.bn(x)
        x = self.activation(x)
...
class ResnetBBlock(BaseModule):
    ...
    def __init__(
        ...
        self.kp_conv = SimpleBlock(
            ...
            add_one=add_one,
            sharp_conv=sharp_conv,
            sharp_conv_agg=sharp_conv_agg,
            sharp_neighbor_pos=sharp_neighbor_pos,
            **kwargs,
        )
