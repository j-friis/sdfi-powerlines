...
class Denmark(Dataset):
    def __init__(self, root, split, block_size, overlap: float, global_z=None,
                 transform=None, pre_transform=None, pre_filter=None, varHeight=False):
        ...
        self.processed_split_folder = (Path(root) / "processed_var_1" / f"{split}_{overlap}_{block_size}")
    ...
    def process(self):
        ...
        for points, coord_min, coord_max in zip(room_points, room_coord_min, room_coord_max):
            ...
            ## judge by the variance of the height
        ...
        for room_i in tqdm(range(len(n_point_rooms))):
            ...
            for i, j in tqdm(product(range(n_split_2d[0]), range(n_split_2d[1]))):
                ...
                if len(point_idx) > 0:
                    ...
                    # ipdb.set_trace()
                    ## option1: judge the variance after normalize
                    if self.varHeight == True:
                        if np.var(points[::,-1] * room_coord_scale[room_i][2]) < 2:
                            var_count += 1
                            continue
                        print(var_count)
    
                    torch.save({"filename": room_i, "coord_min": coord_min, "coord_max":coord_max,
                                "points": points, "labels": labels, "room_idx": room_i, "part_i": i, "part_j": j},
                                self.processed_split_folder / f"{room_i}_cloud_{counter}.pt")
...
class DenmarkDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        self.train_dataset = Denmark(
            ...
            transform=self.train_transform, pre_transform=self.pre_transform, varHeight =True,
        )
        ...
        self.val_dataset = Denmark(
            ...
            transform=self.val_transform, pre_transform=self.pre_transform, varHeight = False,
        )

        self.test_dataset = Denmark(
            split='test', root=self._data_path, overlap=0,
            block_size=block_size, global_z=self.train_dataset.global_z,
            transform=self.test_transform, pre_transform=self.pre_transform, varHeight = False,
        )
