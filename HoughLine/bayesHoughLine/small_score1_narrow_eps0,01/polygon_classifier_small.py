...
class TemplateClassifier(BaseEstimator, ClassifierMixin):
    ...
    def score(self, _, __):
        ...
        for i, laz_file in enumerate(laz_point_cloud_paths):
            ...
            items = [i, laz_file, pct_kept_powerline, pct_lost_datapoints, amount_points, amount_wire, new_amount_points, amount_wire-new_amount_wire, self.get_params()]
            for item in items[:-1]:
                file.write(str(item)+",")
        ...
        score = 0
        if 1-np.mean(pct_kept_powerline_list) <= epsilon:
            score = np.mean(pct_lost_datapoints_list)
...
if __name__ == "__main__":
    ...
    params = {
            ...
            "canny_lower": Integer(20,36),
            "canny_upper": Integer(156,172),
            "hough_lines_treshold": Integer(66,82),
            "min_line_length": Integer(4,8),
            "max_line_gap": Integer(15,25),
            "closing_kernel_size": [1],
            "opening_kernel_size": [1],
            "small_dialation_kernel": [5],
            "meters_around_line": Integer(4,7),
            "simplify_tolerance": [8],
            "cc_area": Integer(5000,6000),
